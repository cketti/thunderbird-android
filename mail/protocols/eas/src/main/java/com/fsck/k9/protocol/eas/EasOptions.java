...
import com.fsck.k9.protocol.eas.util.Sets;
...
public class EasOptions extends EasOperation {
    ...
    private static final Set<String> SUPPORTED_PROTOCOL_VERSIONS = Sets.newUnmodifiableSet(
    ...
    EasOptions(final EasOperation parentOperation) {
        super(parentOperation);
    }

    public EasOptions(Context context, Account account) {
     ...
     */
    public int getProtocolVersionFromServer() throws MessagingException {
    ...
    protected int handleResponse(final EasResponse response) {
        final String commands = response.getHeader("MS-ASProtocolCommands");
        final String versions = response.getHeader("ms-asprotocolversions");
    ...
    protected Request makeRequest() throws IOException, MessageInvalidException {
        return mConnection.makeOptions().build();
    }
    /**
     * Find the best protocol version to use from the header.
     * @param versionHeader The header value for the server's supported versions.
     * @return The best protocol version we mutually support, or null if none found.
     */
    private String getProtocolVersionFromHeader(final String versionHeader) {
        // The string is a comma separated list of EAS versions in ascending order
        // e.g. 1.0,2.0,2.5,12.0,12.1,14.0,14.1
        LogUtils.d(LOG_TAG, "Server supports versions: %s", versionHeader);
        final String[] supportedVersionsArray = versionHeader.split(",");
