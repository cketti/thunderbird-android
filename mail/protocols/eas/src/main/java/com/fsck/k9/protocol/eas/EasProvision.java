...
import com.fsck.k9.protocol.eas.adapter.ProvisionParser;
...
public class EasProvision extends EasOperation {
    ...
    public EasProvision(Context context, Account account) {
        super(context, account, new EasServerConnection(context, account, account.getOrCreateHostAuthRecv(context)));
        mPolicy = null;
        mPolicyKey = null;
        mStatus = null;
        mPhase = 0;
    ...
    private int performInitialRequest() throws MessagingException {
        mPhase = PHASE_INITIAL;
        return performOperation();
    }

    private void performAckRequestForWipe() throws MessagingException {
        mPhase = PHASE_WIPE;
        performOperation();
    }

    private int performAckRequest(final boolean isPartial) throws MessagingException {
     ...
     */
    public final Policy test() throws MessagingException {
     ...
     */
    public final int provision() throws MessagingException {
        ...
        if (result == RESULT_POLICY_SUPPORTED && !mAccount.shouldHandleFullProvisioning()) {
            mAccount.getPolicyManager().setAccountPolicy(mPolicy);
            return RESULT_ABORT;
        }

        // Acknowledge to the server and make sure all's well.
        int ackResult = performAckRequest(result == RESULT_POLICY_UNSUPPORTED);
        if (ackResult == RESULT_POLICY_UNSUPPORTED) {
            throw new PolicyException(mPolicy);
        }
        ...
        if (!mAccount.shouldHandleFullProvisioning()) {
            return RESULT_ABORT;
        }
    ...
    @Override
    protected int handleResponse(final EasResponse response) throws IOException, MessagingException {
