...
import com.fsck.k9.protocol.eas.util.RedirectFollowInterceptor;
import okhttp3.Call;
...
public class EasServerConnection {
    ...
    private OkHttpClient getOkHttpClient(final long timeout) throws CertificateException {
        if (mClient == null) {
            mClient = OkHttpClientProvider.getOkHttpClient().newBuilder()
                    .connectTimeout(CONNECTION_TIMEOUT, TimeUnit.MILLISECONDS)
                    .readTimeout(timeout, TimeUnit.MILLISECONDS)
                    .writeTimeout(timeout, TimeUnit.MILLISECONDS)
                    .followRedirects(false)
                    .addInterceptor(new RedirectFollowInterceptor(MAX_REDIRECTS))
                    .build();
