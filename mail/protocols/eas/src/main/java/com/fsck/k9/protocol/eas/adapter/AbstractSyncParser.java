...
import com.fsck.k9.protocol.eas.FolderSyncRequiredException;
import com.fsck.k9.protocol.eas.LogUtils;
import com.fsck.k9.protocol.eas.callback.BaseSyncCallback;
...
public abstract class AbstractSyncParser extends Parser {
    ...
    private BaseSyncCallback callback;

    private boolean mLooping;

    public AbstractSyncParser(InputStream in, BaseSyncCallback callback) throws IOException {
        super(in);
    ...
    public boolean parse() throws IOException, CommandStatusException, FolderSyncRequiredException {
        ...
        while (nextTag(START_DOCUMENT) != END_DOCUMENT) {
            if (tag == Tags.SYNC_COLLECTION || tag == Tags.SYNC_COLLECTIONS) {
                ...
                if (callback.isFirstSync()) {
                    moreAvailable = true;
                }
                String newKey = getValue();

                if (callback.setSyncKey(newKey)) {
                    newSyncKey = true;
                }
            } else {
                skipTag();
